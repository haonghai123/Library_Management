...
if (isset($_POST['add_category'])) {
	...
	if (empty($_POST['category_name'])) {
		$error .= '<li>Phải có tên danh mục</li>';
	} else {
	...
	if ($error == '') {
		...
		if ($statement->rowCount() > 0) {
			$error = '<li>Tên thể loại đã tồn tại</li>';
		} else {
			$data = array(
...
if (isset($_POST["edit_category"])) {
	...
	if (empty($_POST["category_name"])) {
		$error .= '<li>Phải có tên thể loại</li>';
	} else {
...
<div class="container-fluid py-4" style="min-height: 700px;">
	...
	?>
			...
			<ol class="breadcrumb mt-4 mb-4 bg-light p-2 border">
				...
				<li class="breadcrumb-item"><a href="category.php">Quản lí thể loại</a></li>
				<li class="breadcrumb-item active">Thêm danh thể loại</li>
			</ol>
			<div class="row">
				<div class="col-md-6">
					<?php
					...
					<div class="card mb-4">
						<div class="card-header">
							<i class="fas fa-user-plus"></i> Thêm mới thể loại
						</div>
						<div class="card-body">
							...
							<form method="POST">
								...
								<div class="mt-4 mb-0">
									<input type="submit" name="add_category" value="Thêm" class="btn btn-success" />
								</div>
			...
			?>
					...
					<div class="row">
						<div class="col-md-6">
							<div class="card mb-4">
								<div class="card-header">
									<i class="fas fa-user-edit"></i> Chỉnh sửa chi tiết thể loại
								</div>
								<div class="card-body">
									...
									<form method="post">
										...
										<div class="mt-4 mb-0">
											...
											<input type="submit" name="edit_category" class="btn btn-primary" value="Chỉnh sửa" />
										</div>
		...
		if (isset($_GET['msg'])) {
			if ($_GET['msg'] == 'add') {
				echo '<div class="alert alert-success alert-dismissible fade show" role="alert">Đã thêm thể loại<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
			}

			if ($_GET["msg"] == 'edit') {
				echo '<div class="alert alert-success alert-dismissible fade show" role="alert">Đã chỉnh sửa thể loại <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
			}
			if ($_GET["msg"] == 'disable') {
			...
			<div class="card-header">
				<div class="row">
					...
					<div class="col col-md-6" align="right">
						<a href="category.php?action=add" class="btn btn-success btn-sm">Thêm</a>
					</div>
				</div>
			</div>
			<div class="card-body">
				...
				<table id="datatablesSimple">
					...
					<tbody>
						...
						if ($statement->rowCount() > 0) {
							foreach ($statement->fetchAll() as $row) {
								...
								if ($row['category_status'] == 'Enable') {
									$category_status = '<div class="badge bg-success">Bật</div>';
								} else {
									$category_status = '<div class="badge bg-danger">Tắt</div>';
								}
						...
						<tr>
							...
							<td>
								<a href="category.php?action=edit&code=' . convert_data($row["category_id"]) . '" class="btn btn-sm btn-primary">Chỉnh sửa</a>
								<button name="delete_button" class="btn btn-danger btn-sm" onclick="delete_data(`' . $row["category_id"] . '`, `' . $row["category_status"] . '`)">Xóa</button>
							</td>
						</tr>
						';
							}
				...
				<script>
					function delete_data(code, status) {
						...
						if (confirm("Bạn chắc chắn có muốn " + new_status + " Thể loại này?")) {
